...
namespace Nez.Tweens
{
	...
	/// in a single, cacheable class
	/// </summary>
	public class TransformVector2BezierTween : Vector2Tween, ITweenTarget<Vector2>
	{
		Transform _transform;
		TransformTargetType _targetType;
		private readonly Vector2[] targets;

		public TransformVector2BezierTween() { 		}
		public TransformVector2BezierTween(Vector2[] targets)
		{
		...
		protected override void UpdateValue()
		{
			...
			if ((_targetType == TransformTargetType.RotationDegrees ||
				 _targetType == TransformTargetType.LocalRotationDegrees) && !_isRelative)
				throw new NotSupportedException("No way to Bezier angles");
			else
				SetTweenedValue(Lerps.EaseBezier(targets, EaseHelper.Ease(_easeType, _elapsedTime, _duration)));
		}
		...
		public override void RecycleSelf()
		{
			if (_shouldRecycleTween)
			{
				...
				_nextTween = null;
				_transform = null;
				Pool<TransformVector2BezierTween>.Free(this);
			}
		}
	}
}